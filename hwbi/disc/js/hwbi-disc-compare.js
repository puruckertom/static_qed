...
async function scoreAdjacentByFIPS(fips) {
    ...
    let { adjacentCounties, farthestDistance } = getAdjacentCounties(fips, currentCentroid);
            ...
            for (let j = 0; j < currentCountyGeoCoords.length; j++) {
                ...
                for (let k = 0; k < countyGeo.length; k++) {
                    ...
                    if (coord1[0] === coord2[0] 
                        ...
                        let currentDistance = distance(baselineCentroid, comp_path.centroid(countyData));
...
function getAdjacentCounties(fips, baselineCentroid) {
    let { boardingCounties, farthestDistance } = getBoarderingCounties(fips, baselineCentroid);
    let adjacentCounties = [];
    for (let i = 0; i < boardingCounties.length; i++) {
        let { boardingCounties : counties, farthestDistance : distance} = getBoarderingCounties(boardingCounties[i], baselineCentroid);
